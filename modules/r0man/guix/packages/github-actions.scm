      ...
      #~(modify-phases %standard-phases
          ...
          (add-after 'install 'install-launchers
            (lambda* (#:key inputs outputs #:allow-other-keys)
              ...
              ;; Reference the exact store path of Bash instead of
              ;; '#!/bin/bash' or '#!/usr/bin/env bash': the former
              ;; relies on an FHS path that only exists on Guix System
              ;; by accident, the latter resolves Bash via PATH, which
              ;; is impure and unpredictable for daemon users.
              (define bash
